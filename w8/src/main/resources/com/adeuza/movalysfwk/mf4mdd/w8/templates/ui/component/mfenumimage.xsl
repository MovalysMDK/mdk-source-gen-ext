...
<xsl:template match="visualfield[component = 'MFEnumImage']" mode="create-component">

		<xsl:text>&lt;mf:MFEnumImage x:Name="</xsl:text>
		...
		<xsl:text> mf:EnumImage="{Binding Path=</xsl:text>
		...
		<xsl:text>" mf:Prefix="enum"</xsl:text>
		<xsl:text>&#47;&gt;</xsl:text>
