...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFNumberPicker']">
	<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFNumberPickerCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		

		<key>typeName</key>
		<string>MFNumberPickerCell</string>
		...
		<key>fields</key>
		<array>
			<xsl:call-template name="numberpickerFields"/>
		</array>
	</dict>
	</xsl:template>
	
	<xsl:template match="subView[customClass='MFNumberPicker']" mode="fieldForListItem">
		<xsl:call-template name="numberpickerFields"/>
	</xsl:template>
	
	<xsl:template name="numberpickerFields">
		<xsl:apply-templates select="." mode="common-label-generation"/>
		
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFNumberPicker</xsl:with-param>
			</xsl:apply-templates>
