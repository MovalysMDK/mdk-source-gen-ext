...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="class" mode="extra-methods">
		...
		<xsl:for-each
			select="//*[(name() = 'attribute' or name()= 'association') and not(parent::association)]">
			<!-- Function to initialise and add element to any list variables -->
			<xsl:if
				test="name()= 'association' and  (@type='one-to-many' or @type='many-to-many')">
				<xsl:text>&#10;&#10;</xsl:text>
				<xsl:text>/**&#10;</xsl:text>
				<xsl:text>* Add function for the list </xsl:text>
				<xsl:value-of select="@name-capitalized" />
				<xsl:text>&#10;</xsl:text>
				<xsl:text>*/&#10;</xsl:text>
				
				<xsl:text>add</xsl:text><xsl:value-of select="@name-capitalized" /><xsl:text>(obj: </xsl:text><xsl:value-of select="@contained-type-short-name" /><xsl:text>): void {</xsl:text>
				<xsl:text>if (this._</xsl:text><xsl:value-of select="@name" /><xsl:text> === null) {&#10;this._</xsl:text>
