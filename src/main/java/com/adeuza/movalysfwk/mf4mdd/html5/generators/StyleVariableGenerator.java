	...
	private void addWorkspaceColumnsVariable(XProject p_oMProject, Element p_oVariables) {
		...
		for (MScreen oScreen : p_oMProject.getDomain().getDictionnary().getAllScreens()) {
			if(oScreen.isWorkspace()) {
				int workspaceMasterCount=0;
				int workspacePageCount=0;
				int numberOfColumns=0;
				for (MPage oPage : oScreen.getPages()) {
					if (ViewModelType.LIST_1.equals(oPage.getViewModelImpl().getType()) || 
							ViewModelType.LIST_2.equals(oPage.getViewModelImpl().getType()) || 
							ViewModelType.LIST_3.equals(oPage.getViewModelImpl().getType())) {
						workspaceMasterCount++;
					}
					workspacePageCount++;
				}

				numberOfColumns = workspacePageCount-workspaceMasterCount+1;

				workspaceMap.put(oScreen.getName(), numberOfColumns);
			}
		}
