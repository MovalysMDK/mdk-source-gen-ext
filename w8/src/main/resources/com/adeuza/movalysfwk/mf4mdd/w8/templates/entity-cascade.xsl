...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="pojo">
		...
		<xsl:apply-templates select="." mode="file-header">
			<xsl:with-param name="fileName"><xsl:value-of select="class/name"/>Cascade.cs
			</xsl:with-param>
		</xsl:apply-templates>
		...
		<!-- An embedded entity does not have a table, thus its cascade is empty -->
		<xsl:choose>
			<xsl:when test="class/embedded = 'true' or class/@join-class='true'">
				<xsl:text></xsl:text>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:for-each select="//*[(name()= 'association') and not(ancestor::association)]">
			...
			<xsl:value-of select="interface/name"/><xsl:text><![CDATA[> ]]></xsl:text><xsl:value-of
				select="@cascade-name"/>
