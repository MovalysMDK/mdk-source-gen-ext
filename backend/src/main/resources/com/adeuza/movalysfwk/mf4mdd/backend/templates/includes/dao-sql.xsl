...
<xsl:template name="dao-sql-queries">
...
<xsl:template name="dao-sql-queries-initialize">
	<xsl:variable name="interface" select="interface"/>
	
	<!--  selectQuery -->
	selectQuery = new SqlQuery();
	<xsl:text>selectQuery.addFieldToSelect(</xsl:text><xsl:value-of select="dao-interface/name"/>.ALIAS_NAME, 
		<xsl:for-each select="class/identifier/attribute/field | class/identifier/association/field | class/attribute/field | class/association/field | class/attribute//properties/property/field">
	...
	<xsl:for-each select="class/identifier/attribute/field | class/identifier/association/field | class/attribute/field | class/association/field | class/attribute//properties/property/field">
		<xsl:if test="sequence">
			insertQuery.addNextValField(<xsl:value-of select="$interface/name"/>
			<xsl:text>Field.</xsl:text>
			<xsl:value-of select="@name"/>
			<xsl:text>, </xsl:text>
			<xsl:value-of select="/dao/dao-interface/name"/>.<xsl:value-of select="sequence/@name"/>
			<xsl:text>);</xsl:text>
		</xsl:if>
	...
	<!-- Update Query -->
	updateQuery = new SqlUpdate(<xsl:value-of select="dao-interface/name"/>.TABLE_NAME,<xsl:value-of select="dao-interface/name"/>.ALIAS_NAME);
	<xsl:for-each select="class/identifier/attribute/field | class/identifier/association/field | class/attribute/field | class/association/field | class/attribute//properties/property/field">
	...
	<!-- Delete query -->
	deleteQuery = new SqlDelete( <xsl:value-of select="dao-interface/name"/>.TABLE_NAME, <xsl:value-of select="dao-interface/name"/>.ALIAS_NAME );
...
<xsl:template name="dao-sql-getter">
	/**
	...
	public DaoQuery getSelectIdDaoQuery() {
		return new DaoQueryImpl(selectIdQuery.clone(),this.getEntityName());
	}
	</xsl:if>
