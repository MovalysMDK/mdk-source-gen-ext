...
<xsl:template name="dao-getbypk-interface">
...
<xsl:template name="dao-getbypk">
	...
	public <xsl:value-of select="interface/name"/> get<xsl:value-of select="interface/name"/>
		...
		try {			
			...
			<xsl:call-template name="dao-historisation-identifier">
				<xsl:with-param name="interface" select="$interface"/>
			...
			try {
				...
				try {			
					<xsl:call-template name="dao-reference-readresultset-reference">
						<xsl:with-param name="class" select="class"/>
					</xsl:call-template>
				
					while ( oResultSetReader.next()) {
						r_o<xsl:value-of select="interface/name"/> = valueObject(oResultSetReader, p_oDaoQuery, p_oDaoSession, p_oCascadeSet, p_oContext );
					}
				} finally {
