...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="class" mode="define-property">
        ...
        <xsl:text>/**&#10;* define and initialize all of the attributes of the class </xsl:text><xsl:value-of
            select="../../name"/>
        <xsl:text>  &#10;*/</xsl:text>
        ...
        <xsl:for-each select="//*[(name() = 'attribute' or name()= 'association') and not(parent::association)]">
            ...
            <xsl:call-template name="non-generated-bloc">
                <xsl:with-param name="blocId">
                    <xsl:text>attribute-</xsl:text><xsl:value-of select="@name"/><xsl:text>-settings</xsl:text>
                </xsl:with-param>
    ...
    <xsl:template match="*" mode="define-getter-method">
        ...
        <xsl:variable name="defineGetterAttribute">
            <xsl:text>getter-</xsl:text><xsl:value-of select="@name"/>
        </xsl:variable>
    ...
    <xsl:template match="*" mode="define-setter-method">
        ...
        <xsl:if test="((name() = 'attribute' and not(parent::association)) or (name()= 'association' and  (@type='many-to-one' or @type='one-to-one') and not(parent::association))) and not(@derived='true')">
            <xsl:text>_</xsl:text><xsl:value-of select="@name"/><xsl:text> = </xsl:text><xsl:value-of
                select="parameter-name"/><xsl:text>;&#10;</xsl:text>
        </xsl:if>
        <xsl:if test="((name()= 'association' and  (@type='one-to-many' or @type='many-to-many')) and not(parent::association)) and not(@derived='true')">
            <xsl:text>_</xsl:text><xsl:value-of select="@name"/><xsl:text> = </xsl:text><xsl:value-of
                select="parameter-name"/><xsl:text>;&#10;</xsl:text>
        </xsl:if>
        ...
        <xsl:call-template name="non-generated-bloc">
            <xsl:with-param name="blocId">
                <xsl:text>setter-</xsl:text><xsl:value-of select="@name"/>
            </xsl:with-param>
            <xsl:with-param name="defaultSource">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:apply-templates select="../attribute"
                                             mode="call-derived-attr-update"></xsl:apply-templates>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:with-param>
        </xsl:call-template>
