	...
	<xsl:template match="xib-container">
		...
		<document type="com.apple.InterfaceBuilder3.CocoaTouch.XIB" version="3.0" toolsVersion="6250" systemVersion="13E28" targetRuntime="iOS.CocoaTouch" propertyAccessControl="none" useAutolayout="YES" launchScreen="NO" useTraitCollections="YES">
			...
			<objects>
				...
				<view contentMode="scaleToFill" hidesAccessoryWhenEditing="NO" indentationLevel="1" indentationWidth="0.0" 
				customClass="UIView">
					<xsl:attribute name="reuseIdentifier"><xsl:value-of select="name"/></xsl:attribute>
					<xsl:attribute name="customClass"><xsl:value-of select="view-expandablelist-section-name"/></xsl:attribute>
					<xsl:attribute name="id"><xsl:value-of select="name"/>-sectionView</xsl:attribute>
		            ...
		            <autoresizingMask key="autoresizingMask" flexibleMaxX="YES" flexibleMinY="YES"/>	
						...
						<constraints>
							<xsl:apply-templates select="$filteredComponentList" mode="constraints-generation">
								...
								<xsl:with-param name="additionalMargin"><xsl:value-of select="25"/></xsl:with-param>
								<xsl:with-param name="containerId"><xsl:value-of select="name"/>-sectionView</xsl:with-param>
