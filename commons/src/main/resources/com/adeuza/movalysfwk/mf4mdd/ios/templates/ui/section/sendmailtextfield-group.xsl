...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFSendEmailTextField']">
		<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFSendEmailTextFieldCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		

			<key>typeName</key>
			<string>MFSendEmailTextFieldCell</string>
			...
			<key>fields</key>
			<array>
				<xsl:call-template name="sendmailFields"/>
			</array>
		</dict>
	</xsl:template>
	
	<xsl:template match="subView[customClass='MFSendEmailTextField']" mode="fieldForListItem">
		<xsl:call-template name="sendmailFields"/>
	</xsl:template>
	
	<xsl:template name="sendmailFields">
	<xsl:apply-templates select="." mode="common-label-generation"/>
				
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFSendEmailTextField</xsl:with-param>
			</xsl:apply-templates>
