...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
	...
	<xsl:apply-templates select="/diagram/classes/class[uml-name=$class-name]" mode="csvOptionType"/>
</xsl:for-each>
...
<xsl:template match="field|attribute|property" mode="csvOption">
	...
	<xsl:choose>
		<xsl:when test="$smallTypeShortName='boolean'"><xsl:text>yes[1];no[0];#</xsl:text></xsl:when>
