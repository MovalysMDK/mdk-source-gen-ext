...
<xsl:template name="dao-deletelist-interface">
	 ...
	 */
	public void deleteList<xsl:value-of select="$interface/name"/>( BOList&lt;<xsl:value-of select="$interface/name"/>&gt; p_list<xsl:value-of select="$interface/name"/>
...
<xsl:template name="dao-deletelist">
	...
	public void deleteList<xsl:value-of select="interface/name"/>( BOList&lt;<xsl:value-of select="interface/name"/>&gt; p_list<xsl:value-of select="interface/name"/>
		...
		<xsl:if test="count(class/identifier/descendant::attribute) = 1">
			...
			this.dynamicalFieldDao.deleteAllDynamicalFieldsForEntities(<xsl:value-of select="interface/name"/>.ENTITY_NAME.toLowerCase(),p_list<xsl:value-of select="interface/name"/>,p_oContext);
			*/
		</xsl:if>
		}
	}
