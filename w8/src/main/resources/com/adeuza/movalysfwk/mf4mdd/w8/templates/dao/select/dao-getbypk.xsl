...
<xsl:template match="dao" mode="get-by-pk-sync-async">
	...
	public <xsl:if test="$async='true'">async Task&lt;</xsl:if><xsl:value-of select="./interface/name"/><xsl:if test="$async='true'">&gt;</xsl:if> Get<xsl:value-of select="./class/name"/><xsl:if test="$async='true'">Async</xsl:if>
        ...
        <xsl:if test="count(./class/descendant::association[not(@transient='true')])>0">if (r_o<xsl:value-of select="./class/name"/> != null) {</xsl:if>
			...
			if (p_oCascadeSet.Contains(<xsl:value-of select="$class-name"/>Cascade.<xsl:value-of select="@cascade-name"/>)) {
				<xsl:choose>
					<xsl:when test="@type='many-to-many' and join-table">
						...
						r_o<xsl:value-of select="$class-name"/>.<xsl:value-of select="@name-capitalized"/><xsl:text> = </xsl:text><xsl:value-of select="./dao/bean-ref"/><xsl:text>.GetList</xsl:text>
						<xsl:choose>
						...
						<xsl:text>(r_o</xsl:text><xsl:value-of select="$class-name"/><xsl:text>, p_oCascadeSet, p_oEntitySession, p_oContext);</xsl:text>
					</xsl:when>					
					<xsl:otherwise>
							<xsl:value-of select="./dao/name"/><xsl:text> </xsl:text><xsl:value-of select="./dao/bean-ref"/> = (<xsl:value-of select="./dao/name"/>)ClassLoader.GetInstance().GetBean&lt;<xsl:value-of select="./dao-interface/name"/>&gt;();
						<xsl:choose>								
							...
							<xsl:otherwise>
								...
								<xsl:if test="$async='true'">await </xsl:if>
								<xsl:choose>
									<!-- Here is managed the case one-to-one when the current classe hold the foreign key -->
									<xsl:when test="@relation-owner='true'">
										<xsl:value-of select="./dao/bean-ref"/>.Get<xsl:value-of select="./class/name"/><xsl:if test="$async='true'">Async</xsl:if>(<xsl:text>r_o</xsl:text><xsl:value-of select="$class-name"/>.<xsl:value-of select="./@name-capitalized"/>.<xsl:value-of select="../identifier/descendant::attribute[1]/@name-capitalized"/><xsl:text>, p_oCascadeSet, p_oEntitySession, p_oContext</xsl:text>
									</xsl:when>
        ...
        <xsl:if test="count(./class/descendant::association[not(@transient='true')])>0">}</xsl:if>
		<xsl:text>&#13;</xsl:text>
		<xsl:call-template name="non-generated-bloc">
