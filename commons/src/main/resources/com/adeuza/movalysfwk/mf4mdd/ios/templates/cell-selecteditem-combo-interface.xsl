...
<xsl:apply-templates select="." mode="file-header">
	<xsl:with-param name="fileName"><xsl:value-of select="selectedItemCellClassName"/>.h</xsl:with-param>
</xsl:apply-templates>

@interface <xsl:value-of select="selectedItemCellClassName"/> : &#13;
...
<xsl:apply-templates select="sections/section/subViews/subView[@xsi:type='miosEditableView' and (localization='DEFAULT' or localization='DETAIL')]" mode="propertiesForCellInterface"/>
