...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFRegularExpressionTextField']">
		<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFRegularExpressionTextField<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		
			<key>typeName</key>
			<string>MFRegularExpressionTextField</string>
			...
			<key>fields</key>
			<array>
				<xsl:call-template name="regexpFields"/>
			</array>
		</dict>
	</xsl:template>
	
	
	<xsl:template match="subView[customClass='MFRegularExpressionTextField']" mode="fieldForListItem">
		<xsl:call-template name="regexpFields"/>
	</xsl:template>
	
	<xsl:template name="regexpFields">
	<xsl:apply-templates select="." mode="common-label-generation"/>
		
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFRegularExpressionTextField</xsl:with-param>
			</xsl:apply-templates>
