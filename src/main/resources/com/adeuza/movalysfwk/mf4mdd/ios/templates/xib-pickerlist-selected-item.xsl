...
<xsl:output method="xml" indent="yes"/>
	...
	<xsl:template match="xib-container">
		...
		<xsl:variable name="filteredComponentList" select="/xib-container/components/component[(localization='DEFAULT' or localization = 'DETAIL')]"/>
		<xsl:comment> [xib-pickerlist-item.xsl] xib-container(name=<xsl:value-of select="name"/>) </xsl:comment>
		...
		<document type="com.apple.InterfaceBuilder3.CocoaTouch.XIB" version="3.0" toolsVersion="6250" systemVersion="13E28" targetRuntime="iOS.CocoaTouch" propertyAccessControl="none" useAutolayout="YES" launchScreen="NO" useTraitCollections="YES">
			...
			<objects>
				...
				<view contentMode="scaleToFill" customClass="MFCell1ComponentHorizontal">
					...
					<xsl:attribute name="customClass"><xsl:value-of select="pickerlist-selected-item-name"/></xsl:attribute>
		            ...
		            <autoresizingMask key="autoresizingMask" flexibleMaxX="YES" flexibleMinY="YES"/>
						...
						<constraints>
							<xsl:apply-templates select="$filteredComponentList" mode="constraints-generation">
								...
								<xsl:with-param name="filterNodeValue">DETAIL</xsl:with-param>
							</xsl:apply-templates>
						</constraints>
					<connections>
