...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFTextView']">
	<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFTextViewCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		

		<key>typeName</key>
		<string>MFTextViewCell</string>

		<key>configurationName</key>
		<string>mfmultilinetextcell</string>
		...
		<key>fields</key>
		<array>
			<xsl:call-template name="textViews"/>
		</array>
	</dict>
	</xsl:template>
	
	<xsl:template match="subView[customClass='MFTextView']" mode="fieldForListItem">
		<xsl:call-template name="textViews"/>
	</xsl:template>
	
	<xsl:template name="textViews">
	<xsl:apply-templates select="." mode="common-label-generation"/>
				
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFTextView</xsl:with-param>
			</xsl:apply-templates>
