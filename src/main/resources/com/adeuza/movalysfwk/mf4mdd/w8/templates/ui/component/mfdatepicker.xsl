...
<xsl:template match="visualfield[component = 'MFDatePicker']" mode="create-component">
		<xsl:text>&lt;mf:MFDatePicker x:Name="</xsl:text>
		...
		<xsl:text> mf:Mode="</xsl:text>
		<xsl:value-of select="uml-name"/>
		<xsl:text>" mf:Value="{Binding Path=</xsl:text>
		<xsl:value-of select="property-name-c" />
		<xsl:text>.Value, Converter={StaticResource DateTimeToDateTimeOffsetConverter}, Mode=TwoWay}"</xsl:text>
