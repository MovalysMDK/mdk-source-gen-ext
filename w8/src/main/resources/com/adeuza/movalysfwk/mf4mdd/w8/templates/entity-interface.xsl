...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="pojo">
		...
		<xsl:apply-templates select="." mode="file-header">
			<xsl:with-param name="fileName"><xsl:value-of select="interface/name"/>.cs
			</xsl:with-param>
		</xsl:apply-templates>
		...
		<xsl:if test="class/transient != 'true' or class/embedded != 'true'">
		...
		<xsl:choose>
			<xsl:when test="class/embedded = 'true'">
		...
		<xsl:if test="class/stereotypes/stereotype[@name = 'Mm_model'] or class/@join-class = 'true'">
			<xsl:for-each select="//*[((name() = 'attribute' and not(parent::association)))]">
				...
				<xsl:if test="field and (not(//class/embedded) or //class/embedded != 'true')">
					<xsl:choose>
						<xsl:when
								test="parent::identifier and (not(//class/@join-class) or //class/@join-class != 'true')">
				...
				<xsl:choose>
					<xsl:when test="name() = 'association'">
						<xsl:value-of select="attribute/@type-short-name"/><xsl:text> </xsl:text><xsl:value-of
							select="@name-capitalized"/><xsl:value-of select="attribute/@name-capitalized"/>
					</xsl:when>
					<xsl:when
							test="parent::identifier and //class/@join-class != 'true' or //class/stereotypes/stereotype[@name = 'Mm_Photo']">
						<xsl:text>new </xsl:text><xsl:value-of select="@type-short-name"/><xsl:text> </xsl:text><xsl:value-of
							select="@name-capitalized"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="@type-short-name"/><xsl:text> </xsl:text><xsl:value-of
							select="@name-capitalized"/>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:text>{</xsl:text>
		...
		<xsl:for-each select="//*[(name() = 'association' and not(parent::association))]">
			<xsl:if test="@type='one-to-one' or @type='many-to-one'">
				<xsl:text>[ForeignKey("</xsl:text><xsl:value-of select="field/@name"/><xsl:text>","</xsl:text><xsl:value-of
					select="attribute/field/@name"/><xsl:text>")]&#13;</xsl:text>
			</xsl:if>
