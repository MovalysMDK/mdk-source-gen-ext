...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFBrowseUrlTextField']">
		<dict>
			...
			<xsl:if test="../../@isNoTable='false'">
				...
				<string>MFBrowseUrlTextFieldCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
			</xsl:if>
			
			<key>typeName</key>
			<string>MFBrowseUrlTextFieldCell</string>
			...
			<key>fields</key>
			<array>
				<xsl:call-template name="browseUrlFields"/>
			</array>
			
		</dict>
	</xsl:template>
	
	<xsl:template match="subView[customClass='MFBrowseUrlTextField']" mode="fieldForListItem">
		<xsl:call-template name="browseUrlFields"/>
	</xsl:template>
	
	<xsl:template name="browseUrlFields">
		<xsl:apply-templates select="." mode="common-label-generation"/>
		
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFBrowseUrlTextField</xsl:with-param>
			</xsl:apply-templates>
