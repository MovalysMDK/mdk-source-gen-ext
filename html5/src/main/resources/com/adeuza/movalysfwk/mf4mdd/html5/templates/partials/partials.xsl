...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="view[@isScreen='false' and @is-list='true']" mode="partial-hmtl">
		<div>
			<xsl:attribute name="class"> mdk-well  panel-body panel-list-container</xsl:attribute>
	...
	<xsl:if test="@isScreen='true'">
		...
		<xsl:choose>
			...
			<div>
				...
				<xsl:attribute name="class"><xsl:value-of select="./name"/>-workSpace-column </xsl:attribute>
				<xsl:apply-templates select="./nestedSubviews/workSpaceColumn" mode="display-workSpace-structure"/>
			</div>
...
<xsl:template match="masterColumn" mode="display-master-structure">
	<div>
		<xsl:attribute name="id">firstColumn</xsl:attribute>
		<xsl:attribute name="class"> <xsl:value-of select="../../name"/>-master-column </xsl:attribute>
		<xsl:apply-templates select="./section" mode="display-section-structure"/>
	</div>
</xsl:template>
