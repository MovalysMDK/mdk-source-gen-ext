...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="screen[workspace='true']" mode="superclass-import">
		...
		<import>com.adeuza.movalysfwk.mobile.mf4android.activity.AbstractWorkspaceMasterDetailMMActivity</import>
		<import>com.adeuza.movalysfwk.mobile.mf4android.activity.AbstractWorkspaceDetailMMActivity</import>
	...
	<xsl:template match="pages/page/adapter[ancestor::screen/workspace='true']" 
		...
		oActionParameter.addAdapter(this.getListViewByResId(R.id.<xsl:value-of select="../mastercomponentname"/>).getListAdapter());
	</xsl:template>
	...
	<xsl:template match="screen[workspace='true' and descendant::page/actions/action/action-type='SAVEDETAIL']" mode="do-keep-modifications">
		@Override
	...
	<xsl:template match="screen[workspace='true']" mode="extra-methods">
		...
		<xsl:apply-templates select="pages/page[parameters/parameter[@name='workspace-panel-type'] = 'master']" mode="selected-item-method"/>

		<xsl:apply-templates select="self::node()[descendant::page/actions/action/action-type='SAVEDETAIL']" mode="do-keep-modifications"/>
		...
		<xsl:if test="not(descendant::page/actions/action/action-type='SAVEDETAIL')">
			@Override
	...
	<xsl:template match="page" mode="selected-item-method">
		/**
		 * {@inheritDoc}
		 */
		@ListenerOnBusinessNotification(<xsl:value-of select="adapter/name"/>.SelectedItemEvent.class)
		public void doOnMasterListChangeSelectedItem(final <xsl:value-of select="adapter/name"/>.SelectedItemEvent p_oEvent) {
			super.doOnMasterListChangeSelectedItem(p_oEvent);
			<xsl:call-template name="non-generated-bloc">
