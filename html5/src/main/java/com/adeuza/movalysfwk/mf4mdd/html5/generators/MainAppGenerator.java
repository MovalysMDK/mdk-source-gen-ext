...
public class MainAppGenerator extends AbstractInjectionGenerator<MF4HDomain<MF4HDictionary, MF4HModelFactory>> {

	/**
	 * Logger
	 */
	private static final Logger log = LoggerFactory.getLogger(MainAppGenerator.class);
	...
	public void genere(
		...
		log.debug("> MainAppGenerator.genere");
		...
		log.debug("< MainAppGenerator.genere: {}", oChrono.stopAndDisplay());
	...
	private void createMainApp(	XProject<MF4HDomain<MF4HDictionary, MF4HModelFactory>> p_oMProject,
		...
		String sTargetFile = "webapp/src/app/app.js";

		Document xViewModelDoc = DocumentHelper.createDocument(r_xMainAppFile);
		
		//Main injection (of all the states)
		FilePartGenerationConfig oFilePartGenerationConfig = new FilePartGenerationConfig( "app-modulable-state", "mainapp/app.xsl", xViewModelDoc);
		...
		//Second injection (of the urlRouterProvider) app-urlRouterProvider
		FilePartGenerationConfig oFilePartGenerationurlRouterProvider = new FilePartGenerationConfig( "app-urlRouterProvider", "mainapp/app-urlRouterProvider.xsl", xViewModelDoc);

		this.doInjectionTransform(sTargetFile, p_oMProject, p_oGeneratorContext, oFilePartGenerationurlRouterProvider);
