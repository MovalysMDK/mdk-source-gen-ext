...
/**
 * <p>
 * Generate the correct SplashScreenCtrl of the app
 * </p>
 * 
 * <p>
 * Copyright (c) 2011
...
public class SplashScreenCtrlGenerator extends AbstractInjectionGenerator<MF4HDomain<MF4HDictionary, MF4HModelFactory>> {

	/**
	 * 
	 */
	private static final Logger log = LoggerFactory.getLogger(SplashScreenCtrlGenerator.class);
	...
	public void genere( XProject<MF4HDomain<MF4HDictionary, MF4HModelFactory>> p_oMProject, DomainGeneratorContext p_oContext) throws Exception {
		...
		log.debug("> SplashScreenCtrlGenerator.genere");
		...
		if ( !p_oMProject.getDomain().getDictionnary().getAllMH5Views().isEmpty()) {
			...
			String sTargetFile = "webapp/src/app/views/splashscreen/SplashScreenCtrl.js";
			...
			FilePartGenerationConfig oFilePartGenerationConfig = new FilePartGenerationConfig(
					"splashscreen-exitState", "exit-state-filler.xsl", xDoc);
		...
		log.debug("< SplashScreenCtrlGenerator.genere: {}", oChrono.stopAndDisplay());
	}
}