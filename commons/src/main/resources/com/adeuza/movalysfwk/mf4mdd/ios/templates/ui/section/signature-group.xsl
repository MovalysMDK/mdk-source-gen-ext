...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFSignature']">
	<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFSignatureCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		

		<key>typeName</key>
		<string>MFSignatureCell</string>
		...
		<key>fields</key>
		<array>
			<xsl:call-template name="signatureFields"/>
		</array>
	</dict>
	</xsl:template>
	
	<xsl:template match="subView[customClass='MFSignature']" mode="fieldForListItem">
		<xsl:call-template name="signatureFields"/>
	</xsl:template>
	
	<xsl:template name="signatureFields">
	<xsl:apply-templates select="." mode="common-label-generation"/>
				
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFSignature</xsl:with-param>
			</xsl:apply-templates>
