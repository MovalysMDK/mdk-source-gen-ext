...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<xsl:variable name="method-bloc-id"><xsl:value-of select="@name"/><xsl:text>-calc-method</xsl:text></xsl:variable>
			...
			<xsl:call-template name="non-generated-bloc">
				...
				<xsl:with-param name="defaultSource">
					...
					<xsl:apply-templates select="." mode="attribute-setter"/>
					<xsl:text>}&#13;</xsl:text>
				</xsl:with-param>
			</xsl:call-template>
	...
	<xsl:template match="attribute[@type-short-name='MDKPresenterSVMImpl']" mode="attribute-setter">

		<xsl:text>MDKPresenterSVMImpl oPresenter = null;&#13;</xsl:text>

		<xsl:if test="preceding-sibling::*[@type-short-name='String']/@name">

			<xsl:text>oPresenter = new MDKPresenterSVMImpl();&#13;</xsl:text>
			<xsl:text>oPresenter.setTitle(</xsl:text>
			<xsl:value-of select="preceding-sibling::*[@type-short-name='String']/@name"/>
			<xsl:text>);&#13;</xsl:text>

		</xsl:if>
		<xsl:value-of select="set-accessor"/><xsl:text>(oPresenter);&#13;</xsl:text>
		
	</xsl:template>

	<xsl:template match="*" mode="attribute-setter">
		<xsl:value-of select="set-accessor"/><xsl:text>(null);&#13;</xsl:text>
	</xsl:template>
