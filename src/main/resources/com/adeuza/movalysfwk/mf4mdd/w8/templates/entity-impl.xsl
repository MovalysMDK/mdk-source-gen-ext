...
<xsl:template match="class">
	...
	<xsl:choose>
		<xsl:when test="create-from-expandable-processor = 'true'">
			<xsl:value-of select="originClassFromExpandableProcessor"/>
	...
	<xsl:if test="../create-from-expandable-processor = 'true' and @kind = 'basic'">
	...
	<xsl:choose>
		...
		<xsl:when test="name()= 'association'">
			<xsl:text>private </xsl:text><xsl:value-of select="attribute/@type-short-name"/><xsl:text> _</xsl:text><xsl:value-of select="@name"/><xsl:value-of select="attribute/@name-capitalized"/><xsl:text>;&#13;</xsl:text>
		</xsl:when>
		<xsl:otherwise>
	...
	<xsl:if test="name()= 'association'">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:text>get { return _</xsl:text><xsl:value-of select="@name"/><xsl:value-of select="attribute/@name-capitalized"/><xsl:text>; }</xsl:text>
				<xsl:text>set { _</xsl:text><xsl:value-of select="@name"/><xsl:value-of select="attribute/@name-capitalized"/><xsl:text> = value; }</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
		<xsl:text>}</xsl:text>
		<xsl:text>&#13;&#13;</xsl:text>
	...
	<xsl:text>private </xsl:text><xsl:value-of select="@type-short-name"/><xsl:text> _</xsl:text><xsl:value-of select="@name"/><xsl:text>;&#13;</xsl:text>
	<xsl:text>/// &lt;inheritDoc /&gt;&#13;</xsl:text>
	<xsl:text>public </xsl:text><xsl:value-of select="@type-short-name"/><xsl:text> </xsl:text><xsl:value-of select="@name-capitalized"/><xsl:text></xsl:text>
	<xsl:text>{</xsl:text>
		<xsl:choose>
			<xsl:when test="../create-from-expandable-processor = 'true'">
				<xsl:text>get;</xsl:text>
