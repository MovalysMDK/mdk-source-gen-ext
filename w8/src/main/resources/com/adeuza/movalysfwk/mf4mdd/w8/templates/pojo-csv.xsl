...
<xsl:template match="table" mode="csvGeneratorTable">
	...
	<xsl:apply-templates select="/diagram/classes/class[table-name = $tableName]" mode="addCompositeAttribute">
...
<xsl:template match="class" mode="addCompositeAttribute">
	...
	<xsl:if test="count( stereotypes/stereotype[@name = 'adjava_classFromExpandableAttributeStereotype'] ) = 0">
		<xsl:for-each select="attribute[@kind='composite']">
			<xsl:call-template name="csvGeneratorCompositeClass">
	    		...
	    		<xsl:with-param name="cptTable" select="format-number($cptTable + position(), '000')"/>
...
<!-- *** FileName *** -->
<xsl:text>
FileName#User</xsl:text><xsl:value-of select="$cptTable"/><xsl:text>_</xsl:text>
<xsl:choose>
	<xsl:when test="/diagram/classes/class[table-name=$nameTable]/@join-class ='true'">
		<xsl:value-of select="/diagram/classes/class[table-name=$nameTable]/left-association/real-name-for-join-class"/>
		<xsl:text>-</xsl:text>
		<xsl:value-of select="/diagram/classes/class[table-name=$nameTable]/right-association/name"/>
		<xsl:text>_</xsl:text>
		<xsl:value-of select="/diagram/classes/class[table-name=$nameTable]/right-association/real-name-for-join-class"/>
		<xsl:text>-</xsl:text>
		<xsl:value-of select="/diagram/classes/class[table-name=$nameTable]/left-association/name"/>
	</xsl:when>
	<xsl:otherwise>
		<xsl:value-of select="$uml-name"/>
	</xsl:otherwise>
...
<xsl:template name="csvGeneratorClass">
	...
	<!-- Variable : Name Class -->
	<xsl:variable name="uml-name" select="/diagram/classes/class[table-name=$nameTable]/uml-name"/>
	...
	<xsl:variable name="isNotExpandableAttributeClass" select="boolean(count( /diagram/classes/class[table-name=$nameTable]/stereotypes/stereotype[@name = 'adjava_classFromExpandableAttributeStereotype'] )= 0)"/>

<!-- *** Name *** -->
<xsl:text>
Name#</xsl:text>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
...
<xsl:if test="$isNotExpandableAttributeClass">
	...
	<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute[@kind='composite' and properties/property[count(field)>0]]">
		<xsl:value-of select="translate(concat(@name,'identifier'),$minuscules,$majuscules)"/><xsl:text>#</xsl:text>
	</xsl:for-each>
</xsl:if>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute/properties/property[count(field)>0]">
	<xsl:value-of select="translate(@name,$minuscules,$majuscules)"/><xsl:text>#</xsl:text>
</xsl:for-each>
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
	<xsl:apply-templates select="." mode="changeIdToIdentifier"/>
</xsl:for-each>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/association/field">
	<xsl:apply-templates select="." mode="changeIdToIdentifier"/>
</xsl:for-each>
...
<xsl:if test="$isNotExpandableAttributeClass">
	<!--  on cree une seule colonne de lien vers un autre onglet -->
	<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute[@kind='composite' and properties/property[count(field)>0]]">
		<xsl:value-of select="translate(concat(@name,'identifier'), $majuscules , $minuscules )"/><xsl:text>;</xsl:text>
		<xsl:value-of select="translate(concat(@name,'identifier'), $majuscules , $minuscules )"/><xsl:text>#</xsl:text>
	</xsl:for-each>
</xsl:if>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute/properties/property[count(field)>0]">
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
	<xsl:choose>
		<xsl:when test="field/sequence"><xsl:text>champ de type seq;champ de type seq#</xsl:text></xsl:when>
		<xsl:when test="field/@data-type"><xsl:text>champ de type number;champ de type number#</xsl:text></xsl:when>
		<xsl:otherwise><xsl:text>#</xsl:text></xsl:otherwise>
	</xsl:choose>
</xsl:for-each>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/association/field">
	<xsl:choose>
		<xsl:when test="sequence"><xsl:text>champ de type refseq; champ de type refseq#</xsl:text></xsl:when>
		<xsl:when test="@data-type"><xsl:text>champ de type number;champ de type number#</xsl:text></xsl:when>
		<xsl:otherwise><xsl:text>#</xsl:text></xsl:otherwise>
	</xsl:choose>
</xsl:for-each>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute[count(field)>0]">
...
<xsl:text>
VisualSize#</xsl:text>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]"><xsl:text>10#</xsl:text></xsl:for-each>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/association/field"><xsl:text>20#</xsl:text></xsl:for-each>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute[count(field)>0]">
...
<xsl:choose>
	<xsl:when test="$isNotExpandableAttributeClass">
		...
		<!-- on ajoute tous les attributs avec un size à 0 pour les cacher dans la feuille excel et ne pas les saisir et exporter -->
		<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute/properties/property[count(field)>0]">
			<xsl:text>0#</xsl:text>
		</xsl:for-each>	
	</xsl:when>
...
<xsl:if test="$isNotExpandableAttributeClass"><!-- on ajoute tous les attributs -->
	<!--  on cree une seule colonne de lien vers un autre onglet -->
	<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute[@kind='composite' and properties/property[count(field)>0]]">
		<xsl:text>#</xsl:text><!--  pas de nom de colonne pour ne pas générer l insert dans la colonne SQL -->
	</xsl:for-each>
</xsl:if>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute/properties/property/field">
	<xsl:value-of select="translate(@name,$minuscules,$majuscules)"/><xsl:text>#</xsl:text>
</xsl:for-each>
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/association[count(field)>0]">
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute/properties/property">
	<xsl:apply-templates select="." mode="csvMandatory"/>
</xsl:for-each>
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
	<xsl:choose>
		<xsl:when test="field/sequence"><xsl:text>seq#</xsl:text></xsl:when>
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/association/field">
	<xsl:choose>
		<xsl:when test="sequence"><xsl:text>refseq#</xsl:text></xsl:when>
		<!-- <xsl:when test="@data-type"><xsl:text>number#</xsl:text></xsl:when>-->
		<xsl:when test="../@type='many-to-one' or (../@type='one-to-one' and ../@relation-owner='true'  and ../@transient='false')"><xsl:text>ref1#</xsl:text></xsl:when>
		<xsl:otherwise><xsl:text>#</xsl:text></xsl:otherwise>
	</xsl:choose>
</xsl:for-each>
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/attribute[count(field)>0]">
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
	<xsl:choose>
		<xsl:when test="field/sequence"><xsl:text>seq#</xsl:text></xsl:when>
		<!-- <xsl:when test="field/@data-type"><xsl:text>numberid#</xsl:text></xsl:when>
		 --><xsl:otherwise><xsl:text>ref1#</xsl:text></xsl:otherwise>
	</xsl:choose>
</xsl:for-each>
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
	...
	<xsl:variable name="class-name">
		<xsl:value-of select="../../left-association[attr[@name=$attr-name]]/name-for-join-class"/>
		<xsl:value-of select="../../right-association[attr[@name=$attr-name]]/name-for-join-class"/>
	</xsl:variable>
	<xsl:apply-templates select="/diagram/classes/class[translate(uml-name,$majuscules,$minuscules)=$class-name]" mode="csvReferenceFrom"/>
	<xsl:text>#</xsl:text>
</xsl:for-each>
...
<xsl:for-each select="/diagram/classes/class[table-name=$nameTable]/identifier/attribute[count(field)>0]">
	<!--
	<xsl:variable name="nameClass" select="../@type-short-name"/>
	<xsl:apply-templates select="/diagram/classes/classes/class[uml-name=$nameClass]" mode="csvReferenceTo"/>
	-->
	<xsl:variable name="attr-name" select="@name"/>
	<xsl:variable name="class-name">
		<xsl:value-of select="../../left-association[attr[@name=$attr-name]]/name-for-join-class"/>
	...
	<xsl:apply-templates select="/diagram/classes/class[translate(uml-name,$majuscules,$minuscules)=$class-name]" mode="csvReferenceTo"/>
	<xsl:text>#</xsl:text>
</xsl:for-each>
...
<xsl:template name="csvGeneratorCompositeClass">
	 ...
	 <xsl:variable name="classname" select="/diagram/classes/class[table-name=$nameTable]/uml-name"/>
	 
	 <xsl:variable name="uml-name" select="concat($classname,$compositeAttribute/@name-capitalized)"/>
	 <xsl:variable name="relation-inverse" select="concat($compositeAttribute/@name-capitalized,$classname)"/>
	 <xsl:call-template name="csvGeneratorClassHeader">
		<xsl:with-param name="uml-name" select="$uml-name"/>
		<xsl:with-param name="cptTable" select="$cptTable"/>
	</xsl:call-template>
...
<xsl:value-of select="translate($relation-inverse,$minuscules,$majuscules)"/><xsl:text>#</xsl:text>
<!-- *** Module *** -->
<xsl:text>
Module#</xsl:text>
...
<xsl:value-of select="translate($relation-inverse, $majuscules, $minuscules )"/><xsl:text>#</xsl:text>
...
<xsl:apply-templates select="/diagram/classes/class[table-name=$nameTable]" mode="csvReferenceFrom"/><xsl:text>#</xsl:text>
...
<xsl:apply-templates select="/diagram/classes/class[table-name=$nameTable]" mode="csvReferenceTo"/><xsl:text>#</xsl:text>
...
<xsl:text>
Req#U##</xsl:text><xsl:value-of select="$nameTable"/><xsl:text>#</xsl:text>
...
<xsl:template match="field|attribute|property|association" mode="csvMandatory">
	<xsl:choose>
...
<xsl:template match="field|attribute|property" mode="csvSize">
	<xsl:choose>
		<xsl:when test="@length and number(@length) > 0"><xsl:value-of select="@length"/><xsl:text>#</xsl:text></xsl:when>
...
<xsl:template match="field|attribute|property" mode="changeIdToIdentifier">
	...
	<xsl:when test="name(.)='field'">
		<xsl:value-of select="translate(../@name,$majuscules,$minuscules)"/><xsl:text>identifier;</xsl:text>
		<xsl:value-of select="translate(../@name,$majuscules,$minuscules)"/><xsl:text>identifier#</xsl:text>		
	</xsl:when>
	<xsl:otherwise>
