...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFWebView']">
	<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFWebViewCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>

		<key>typeName</key>
		<string>MFWebViewCell</string>
		...
		<key>fields</key>
		<array>
			<xsl:call-template name="webFields"/>
		</array>
	</dict>
	</xsl:template>
	
	<xsl:template match="subView[customClass='MFWebView']" mode="fieldForListItem">
		<xsl:call-template name="webFields"/>
	</xsl:template>
	
	<xsl:template name="webFields">
	<xsl:apply-templates select="." mode="common-label-generation"/>
				
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFWebView</xsl:with-param>
			</xsl:apply-templates>
			...
			<key>parameters</key>
			<dict>
				<xsl:call-template name="webParameters"/>
			</dict>
		</dict>	
	</xsl:template>
	
		<xsl:template name="webParameters">
			<key>defaultUrl</key>
