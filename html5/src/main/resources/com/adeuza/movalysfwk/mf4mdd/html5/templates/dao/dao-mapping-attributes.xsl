	...
	<xsl:template match="attribute[not(@transient='true' or @derived='true' or @type-name='MFAddressLocation' or @type-name='MFPhoto')]" mode="map-attribute">
		...
		<xsl:variable name="leftAttr">
			<xsl:choose>
				...
				<xsl:otherwise><xsl:text>'</xsl:text><xsl:value-of select="translate(@name-uppercase, $uppercase, $smallcase)" /><xsl:text>'</xsl:text></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:template match="association[@relation-owner='true' and not(@type='one-to-many' or @type='many-to-many')]" mode="map-attribute">
		...
		<xsl:variable name="leftAttr">
			<xsl:choose>
				...
				<xsl:otherwise><xsl:text>'</xsl:text><xsl:value-of select="translate(field/@name, $uppercase, $smallcase)"/><xsl:text>'</xsl:text></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
