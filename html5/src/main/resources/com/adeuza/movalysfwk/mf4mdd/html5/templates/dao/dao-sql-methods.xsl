			...
			<xsl:when test="(substring($methodNameToken, 1, 12) = 'saveOrUpdate')">
				<xsl:choose>
					...
					<xsl:when test="($methodParameterToken='p_entity')">
							...
							<xsl:text>			this.exist(p_entity, p_context).then(function(result) {&#10;</xsl:text>
							<xsl:text>				if(result){ &#10;</xsl:text>
			...
			<xsl:when test="(substring($methodNameToken, 1, 4) = 'save')">
				<xsl:choose>
					<xsl:when test="($methodParameterToken='p_entity')">
                        ...
                        <xsl:text>&#10;</xsl:text>
                        <xsl:for-each
                                select="/dao/class/association[((@type='one-to-one') or (@type='many-to-one')) and (@transient='false')]">
							...
							<xsl:if test="position() = 1">
								<xsl:text>if( p_cascadeSet[i].entityName === p_entity._type ) {&#10;</xsl:text>
							</xsl:if>
                            <xsl:text>if( p_cascadeSet[i].key === '</xsl:text><xsl:value-of
                                select="@cascade-name"/><xsl:text>') {&#10;</xsl:text>
                            <xsl:text>			savePointedChildren.push( </xsl:text><xsl:value-of select="dao/name"/><xsl:text>Proxy.saveOrUpdate</xsl:text><xsl:value-of
                                select="class/name"/>
                        ...
                        <xsl:for-each
                                select="/dao/class/association[((@type='one-to-one') or (@type='one-to-many')) and (@transient='false')]">
                            <xsl:if test="position() = 1">
                                <xsl:text>						// --------------------------&#10;</xsl:text>
                            ...
                            <xsl:text>if( p_cascadeSet[i].key === '</xsl:text><xsl:value-of select="@cascade-name"/><xsl:text>') {&#10;</xsl:text>
                            <xsl:choose>
                                <xsl:when test="(@type='one-to-one')">
                                    <xsl:text>							saveOtherChildren.push( </xsl:text><xsl:value-of
                                        select="dao/name"/><xsl:text>Proxy.saveOrUpdate</xsl:text><xsl:value-of
                                        select="class/name"/>
                                    <xsl:text>(p_entity.</xsl:text><xsl:value-of select="@name"/><xsl:text>, p_context, p_cascadeSet, p_toSync) );&#10;</xsl:text>
                                </xsl:when>
                                <xsl:when test="(@type='one-to-many')">
                                    <xsl:text>							saveOtherChildren.push( </xsl:text><xsl:value-of
                                        select="dao/name"/><xsl:text>Proxy.saveOrUpdateList</xsl:text><xsl:value-of
                                        select="class/name"/>
                                    <xsl:text>(p_entity.</xsl:text><xsl:value-of select="@name"/><xsl:text>, p_context, p_cascadeSet, p_toSync) );&#10;</xsl:text>
                                </xsl:when>
                                <xsl:otherwise/>
