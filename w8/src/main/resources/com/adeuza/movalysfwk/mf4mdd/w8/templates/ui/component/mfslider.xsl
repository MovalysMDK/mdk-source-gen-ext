...
<xsl:template match="visualfield[component='MFSlider']" mode="create-component">
		<xsl:text>&lt;mf:MFSlider x:Name="</xsl:text>
		...
		<xsl:text> mf:SliderMaximum="10"</xsl:text>
		<xsl:text> mf:SliderMinimum="0"</xsl:text>
		<xsl:text> mf:SliderStep="1"</xsl:text>
