...
<xsl:template match="controller[@controllerType='COMBOVIEW']"><plist version="1.0">
	<dict>
		<key>typeName</key>
		<string><xsl:value-of select="selectedItemCellClassName"/></string>
		<key>sections</key>
		<array>
			<dict>
				<key>typeName</key>
				...
				<array>
					<dict>
						...
						<string>YES</string>
						<key>name</key>
						<string>g<xsl:value-of select="selectedItemCellClassName"/></string>
						<key>configurationName</key>
						<string></string>
						<key>fields</key>
						<array>
							<xsl:apply-templates select="sections/section/subViews/subView[@xsi:type='miosEditableView' and (localization='DEFAULT' or localization='DETAIL')]" mode="fieldForListItem"/>
						</array>
					</dict>
				</array>
			</dict>
...
<xsl:template match="subView[customClass='UILabel' or customClass='MFLabel']" >
	<xsl:comment>UILabel or MFLabel generation skipped in form-combo-selected-item-plist.xsl</xsl:comment>
</xsl:template>

<xsl:template match="subView" mode="fieldForListItem" priority="-1000">
	<ERROR>GENERATION ERROR in form-combo-selected-item--plist.xsl: not handled when customClass=<xsl:value-of select="customClass"/> and cellType=<xsl:value-of select="cellType"/> and mode="fieldForListItem"</ERROR>
</xsl:template>

<xsl:template match="subView" priority="-1000">
	<ERROR>GENERATION ERROR in form-combo-selected-item--plist.xsl: not handled when customClass=<xsl:value-of select="customClass"/> and cellType=<xsl:value-of select="cellType"/></ERROR>
</xsl:template>

<xsl:template match="subView" mode="gen-table-cell"  priority="-1000">
	<ERROR>GENERATION ERROR in form-combo-selected-item--plist.xsl: not handled when customClass=<xsl:value-of select="customClass"/> and cellType=<xsl:value-of select="cellType"/> and mode="gen-table-cell"</ERROR>
</xsl:template>
</xsl:stylesheet>
