...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFIntegerTextField']">
		<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFIntegerTextFieldCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		

			<key>typeName</key>
			<string>MFIntegerTextFieldCell</string>
			...
			<key>fields</key>
			<array>
				<xsl:call-template name="integerFields"/>
			</array>
		</dict>
	</xsl:template>
	
	<xsl:template match="subView[customClass='MFIntegerTextField']" mode="fieldForListItem">
		<xsl:call-template name="integerFields"/>
	</xsl:template>
	
	<xsl:template name="integerFields">
		<xsl:apply-templates select="." mode="common-label-generation"/>
		
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFIntegerTextField</xsl:with-param>
			</xsl:apply-templates>
