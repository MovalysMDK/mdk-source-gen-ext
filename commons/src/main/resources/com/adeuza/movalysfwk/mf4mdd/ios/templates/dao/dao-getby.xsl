...
* GET BY INTERFACE
* -->
<xsl:template match="method-signature[@type='getEntite' and @by-value='false']" mode="method-signature-for-daointerface">
	<xsl:param name="entityName"/>
/**
 * @brief Get <xsl:value-of select="$entityName"/> matching <xsl:apply-templates select="method-parameter" mode="doxygen-method-brief"/>
 <xsl:text>&#13;</xsl:text>
<xsl:apply-templates select="method-parameter" mode="doxygen-method-param"/>
 * @return <xsl:value-of select="$entityName"/>
 */
<xsl:text>+ (</xsl:text><xsl:value-of select="$entityName"/><xsl:text> *) MF_findFirstBy</xsl:text>
...
<!-- 
* GET BY IMPLEMENTATION
* -->
<xsl:template match="method-signature[@type='getEntite' and @by-value='false']" mode="method-signature-for-daoimpl">
	<xsl:param name="entityName"/>
...
<xsl:text> inContext:(id&lt;MFContextProtocol&gt;) mfContext</xsl:text>
{
	<xsl:call-template name="non-generated-bloc">
		<xsl:with-param name="blocId">MF_findFirstBy</xsl:with-param>
	...
	<xsl:text>[request setFetchLimit:1];</xsl:text>
    ...
    <xsl:text>NSArray *results = [self MR_executeFetchRequest:request inContext:mfContext.entityContext];&#13;</xsl:text>
    <xsl:text>if ([results count] == 0)&#13;</xsl:text>
	<xsl:text>{&#13;</xsl:text>
	<xsl:text>	return nil;&#13;</xsl:text>
	<xsl:text>}&#13;</xsl:text>
	<xsl:text>return [results objectAtIndex:0];&#13;</xsl:text> 
		</xsl:with-param>
	</xsl:call-template>
}
</xsl:template>
