...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFPhotoThumbnail']">
		<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFPhotoThumbnailCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		
			<key>typeName</key>
			<string>MFPhotoThumbnailCell</string>
			...
			<key>fields</key>
			<array>
				<xsl:call-template name="photoFields"/>
			</array>
		</dict>

	</xsl:template>
	
	<xsl:template match="subView[customClass='MFPhotoThumbnail']" mode="fieldForListItem">
		<xsl:call-template name="photoFields"/>
	</xsl:template>
	
	<xsl:template name="photoFields">
<xsl:apply-templates select="." mode="common-label-generation"/>
		
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFPhotoThumbnail</xsl:with-param>
			</xsl:apply-templates>
