...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="page | dialog">
		...
		<xsl:text>  ViewModel { get { return base.viewModel as </xsl:text><xsl:value-of
			select="viewmodel-interface/name" /><xsl:text>; } set { base.viewModel = value; } }&#13;</xsl:text>
	...
	<xsl:template match="reverse-navigationsV2/navigationV2[@type = 'MASTER_DETAIL']" mode="create-event">
		...
		<xsl:text>public void DoOnItemSelected(object sender, </xsl:text><xsl:value-of
		...
		<xsl:text>public void DoOnItemAdd(object sender, </xsl:text><xsl:value-of
		...
		<xsl:text>public void DoOnItemDelete(object sender, </xsl:text><xsl:value-of
	...
	<xsl:template match="navigationsV2/navigationV2[@type = 'MASTER_DETAIL']" mode="create-event">
		...
		<xsl:value-of select="source/component-name-capitalized" /><xsl:text>_SelectionChangedEvent myevent = new </xsl:text><xsl:value-of
			select="source/component-name-capitalized" /><xsl:text>_SelectionChangedEvent(vm.Id_id);</xsl:text>
		<xsl:text>ClassLoader.GetInstance().GetBean&lt;IMFController&gt;().RaiseEventMethods(this, myevent);</xsl:text>
		<xsl:text>}</xsl:text>
		<xsl:text>}</xsl:text>
		<xsl:text>&#13;</xsl:text>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:value-of select="source/component-name-capitalized" /><xsl:text>_AddItemEvent myevent = new </xsl:text><xsl:value-of
					select="source/component-name-capitalized" /><xsl:text>_AddItemEvent(-1);</xsl:text>
				<xsl:text>ClassLoader.GetInstance().GetBean&lt;IMFController&gt;().RaiseEventMethods(this, myevent);</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
		...
		<xsl:text>{</xsl:text>
		<xsl:value-of select="source/component-name-capitalized" /><xsl:text>_DeleteItemEvent myevent = new </xsl:text><xsl:value-of
			select="source/component-name-capitalized" /><xsl:text>_DeleteItemEvent();</xsl:text>
		<xsl:text>ClassLoader.GetInstance().GetBean&lt;IMFController&gt;().RaiseEventMethods(this, myevent);</xsl:text>
		<xsl:text>}&#13;</xsl:text>
		<xsl:text>&#13;/// &lt;inheritDoc/&gt;&#13;</xsl:text>
		<xsl:text>[MFOnEventAttribute(typeof(</xsl:text><xsl:value-of select="source/component-name-capitalized" /><xsl:text>_ReloadEvent))]&#13;</xsl:text>
		<xsl:text>public void DoOnItemReload(object sender, </xsl:text><xsl:value-of
			select="source/component-name-capitalized" /><xsl:text>_ReloadEvent e)</xsl:text>
		<xsl:text>{</xsl:text>
