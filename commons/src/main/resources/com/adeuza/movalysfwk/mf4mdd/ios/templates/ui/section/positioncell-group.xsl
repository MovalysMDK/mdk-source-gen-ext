...
<xsl:include href="ui/section/common-group.xsl"/>
	...
	<xsl:template match="subView[customClass='MFPosition']">
		<dict>
		...
		<xsl:if test="../../@isNoTable='false'">
			...
			<string>MFPositionCell<xsl:if test="@visibleLabel = 'false'">-noLabel</xsl:if></string>
		</xsl:if>
		
		
			<key>typeName</key>
			<string>MFPositionCell</string>
			...
			<key>fields</key>
			<array>
				<xsl:call-template name="positionFields"/>
			</array>
		</dict>

	</xsl:template>
	
	<xsl:template match="subView[customClass='MFPosition']" mode="fieldForListItem">
		<xsl:call-template name="positionFields"/>
	</xsl:template>
	
	<xsl:template name="positionFields">
<xsl:apply-templates select="." mode="common-label-generation"/>
		
		<dict>
			<xsl:apply-templates select="." mode="common-component-generation">
			<xsl:with-param name="type">MFPosition</xsl:with-param>
			</xsl:apply-templates>
