...
<xsl:template name="delete-update-generic">
	...
	<xsl:if test="@type='one-to-many' and @not-null='true'">
	...
	<xsl:if test="@type='one-to-many' and @not-null='false'">
		<xsl:variable name="assoDelete" select="."/>
		<xsl:variable name="source"><xsl:if test="$traitement-list = 'true'"><xsl:text> o</xsl:text><xsl:value-of select="$interface/name"/></xsl:if><xsl:if test="$traitement-list = 'false'"><xsl:value-of select="$object"/></xsl:if></xsl:variable>
		
		SqlUpdate oListSqlUpdate = new SqlUpdate(<xsl:value-of select="dao-interface/name"/>.TABLE_NAME,<xsl:value-of select="dao-interface/name"/>.ALIAS_NAME);<xsl:for-each select="$class/identifier/descendant::attribute">
		...
		SqlEqualsValueCondition oSqlEqualsValueCondition<xsl:value-of select="position()-1"/> = new SqlEqualsValueCondition(<xsl:value-of select="$assoDelete/dao-interface/name"/>.FK_<xsl:value-of select="$assoDelete/@opposite-cascade-name"/>[<xsl:value-of select="position()-1"/>].getKey(),<xsl:value-of select="$assoDelete/dao-interface/name"/>.ALIAS_NAME,
		...
		List&lt;Object&gt; listValues = new ArrayList&lt;Object&gt;();
		final SqlNotInValueCondition oSqlNotInValueCondition = new SqlNotInValueCondition(<xsl:value-of select="dao-interface/name"/>.PK_FIELDS,<xsl:value-of select="dao-interface/name"/>.ALIAS_NAME,listValues);
		if ( !<xsl:value-of select="$source"/><xsl:text>.</xsl:text><xsl:value-of select="get-accessor"/><xsl:text>().isEmpty() ) {</xsl:text>
			for( <xsl:value-of select="interface/name"/> o<xsl:value-of select="interface/name"/>ToAdd : <xsl:value-of select="$source"/><xsl:text>.</xsl:text><xsl:value-of select="get-accessor"/><xsl:text>()) {</xsl:text><xsl:for-each select="descendant::attribute">
				<xsl:if test="parent::association[@type='one-to-many']">listValues.add(o<xsl:value-of select="../interface/name"/>ToAdd.<xsl:value-of select="get-accessor"/>());</xsl:if>
				<xsl:if test="not(parent::association[@type='one-to-many'])">listValues.add(o<xsl:value-of select="../../interface/name"/>ToAdd.<xsl:value-of select="../get-accessor"/>().<xsl:value-of select="get-accessor"/>());</xsl:if></xsl:for-each>
		...
		<!-- 
		<xsl:text>this.</xsl:text><xsl:if test="@self-ref = 'false'"><xsl:value-of select="dao-interface/bean-ref"/><xsl:text>.</xsl:text></xsl:if><xsl:text>genericUpdate( oListSqlUpdate, p_oContext, new StatementBinderCallBack() { 
			@Override
			public void doBind(StatementBinder p_oStatementBinder) throws SQLException, DaoException {</xsl:text><xsl:for-each select="$class/identifier/descendant::attribute">
				p_oStatementBinder.bindNull(SqlType.<xsl:value-of select="jdbc-type"/>);</xsl:for-each><xsl:for-each select="$class/identifier/descendant::attribute">
				oSqlEqualsValueCondition<xsl:value-of select="position()-1"/>.bindValues(p_oStatementBinder);</xsl:for-each>
				oSqlNotInValueCondition.bindValues(p_oStatementBinder);
			}
		});
		 -->
		...
		try {
			...
			oBinder.bindNull(SqlType.<xsl:value-of select="jdbc-type"/>);
			</xsl:for-each>
			<xsl:for-each select="$class/identifier/descendant::attribute">
			oSqlEqualsValueCondition<xsl:value-of select="position()-1"/>.bindValues(oBinder);
			</xsl:for-each>
