...
<xsl:template match="visualfield[component = 'MFWebView']" mode="create-component">

		<xsl:text>&lt;mf:MFWebView x:Name="</xsl:text>
		...
		<xsl:text> mf:Visibility="{Binding Path=</xsl:text>
		<xsl:value-of select="property-name-c" />
