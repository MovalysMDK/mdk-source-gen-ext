...
/**
 * List2D/3D extractor for windows 8
 * @author qlagarde
 *
 */
public class MF4WExpandableListExtractor extends AbstractExtractor<IDomain<IModelDictionary,IModelFactory>> {
	...
	public void initialize(Element p_xConfig) throws Exception {
		//TODO : QLA Voir s'il y a une initialisation spécifique à faire pour cet extracteur.
	}
		...
		for(MViewModelImpl oViewModel : p_oListItem2ViewModel.getSubViewModels()) {
			...
			if(oViewModel.getType() == ViewModelType.LISTITEM_1) {
				...
				String sOldEntityAttributeName = StringUtils.join(this.getParameterValue(PARAMETER_MAPPING_LIST_ENTITY_NAME_PREFIX), 
						oViewModel.getMasterInterface().getName(), 
	 ...
	 * @param p_oChildViewModel The ViewModel that will be an item of this new List ViewModel
