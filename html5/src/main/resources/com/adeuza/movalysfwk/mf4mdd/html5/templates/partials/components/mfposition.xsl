...
<xsl:template match="HTML-attribute[visualfield/component='MFPosition']" mode="partial-component-generation"  priority="1000">
	...
	<xsl:param name="viewModel"/>
		...
		<mf-address-location>
			...
			<xsl:attribute name="mf-latitude-placeholder">{{ 'field.mfaddresslocation.lat.placeholder' | translate }}</xsl:attribute>
			<xsl:attribute name="mf-longitude-placeholder">{{ 'field.mfaddresslocation.lng.placeholder' | translate }}</xsl:attribute>
		</mf-address-location>
</xsl:template>
