...
<xsl:text>.h"&#13;</xsl:text>
	...
	<xsl:text>-(void)registerComponentsInPrototypes:(NSMutableDictionary *)prototypes andSingletons:(NSMutableDictionary *)singletons { &#13;</xsl:text>
...
<xsl:text>[</xsl:text>
 <xsl:choose>
  ...
  <xsl:text>singletons</xsl:text>
  </xsl:when>
  <xsl:when test="./config/scope = 'prototype'">
  <xsl:text>prototypes</xsl:text>
  </xsl:when>
 </xsl:choose>
 <xsl:text> setObject:[</xsl:text>
 <xsl:value-of select="./config/class"></xsl:value-of>
 <xsl:text> class] forKey:@"</xsl:text>
