...
<xsl:template match="action">
	...
	<xsl:apply-templates select="." mode="file-header">
		<xsl:with-param name="fileName">
			<xsl:choose>
				<xsl:when test="./action-type='SAVEDETAIL'">Save</xsl:when>
				<xsl:when test="./action-type='DELETEDETAIL'">Delete</xsl:when>
			</xsl:choose>
	...
	<xsl:apply-templates select="action-type" mode="action-chained-impl" />
