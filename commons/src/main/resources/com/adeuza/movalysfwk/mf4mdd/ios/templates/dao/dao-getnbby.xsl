...
* -->
<xsl:template match="method-signature[@type='getNbEntite' and @by-value='false']" mode="method-signature-for-daointerface">
	<xsl:param name="entityName"/>
/**
 * @brief Count <xsl:value-of select="$entityName"/> matching <xsl:apply-templates select="method-parameter" mode="doxygen-method-brief"/>
 <xsl:text>&#13;</xsl:text>
<xsl:apply-templates select="method-parameter" mode="doxygen-method-param"/>
 * @return <xsl:value-of select="$entityName"/>
 */
<xsl:text>+ (NSUInteger) MF_countBy</xsl:text>
...
<!-- 
* COUNT ENTITY IMPLEMENTATION
* -->
<xsl:template match="method-signature[@type='getNbEntite' and @by-value='false']" mode="method-signature-for-daoimpl">
	<xsl:param name="entityName"/>
...
<xsl:text> inContext:(id&lt;MFContextProtocol&gt;) mfContext</xsl:text>
{
	<xsl:call-template name="non-generated-bloc">
		<xsl:with-param name="blocId">MF_countBy</xsl:with-param>
	...
	<xsl:if test="count(method-parameter) > 1">
		...
		<xsl:text>]];&#13;</xsl:text>
		<xsl:text>return [self MR_countOfEntitiesWithPredicate: predicate inContext:mfContext.entityContext];&#13;</xsl:text>
	</xsl:if>
		</xsl:with-param>
	</xsl:call-template>
}
